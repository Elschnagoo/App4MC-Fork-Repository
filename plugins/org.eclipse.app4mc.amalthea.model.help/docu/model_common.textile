...
The earlier implementation used Generics to support the different use cases. To simplify the usage (via Java API and in the editor) the new implementation provides three different top level interfaces for TimeDeviation, DiscreteDeviation (integer values) and ContinuousDeviation (float values). They provide specialized methods to handle their values and a common interface to access minimum, maximum and average.
The following image shows the detailed hierarchy of discrete deviations, the other implementations are built accordingly.

!(scale)../pictures/model_common_deviations_discrete.png!
