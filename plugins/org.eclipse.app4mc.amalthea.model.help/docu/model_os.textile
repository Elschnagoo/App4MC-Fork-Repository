...
The basic element in the OS Model is the operating system. There can multiple operating systems in one model. The operating system type can be used to describe a generic operating system. It is also possible to use the vendor operating system type to define a vendor specific OS. A operating system contains a number of task schedulers and interrupt controllers. A task scheduler controls the execution of a task on one or multiple processor cores. A interrupt controller is the controller for the execution of ISRs and can be also mapped to multiple cores. The mapping of tasks and ISRs to their controller and the mapping of the controller to the cores can be done in the Mapping Model. An operating system can also contain a description of the overhead it produces. For this there is a more detailed explanation below.  
...
The OS buffering object specifies the sections where the buffering for inter-core communication occurs. The buffering can be done at runnable, process, and schedule section level. When buffering is enabled for a level, the required data for a section of this level (e.g. runnable) is read before its execution starts. After the execution of the section, the data is copied back.

!../pictures/model_os_buffering.png!
...
* API Send Message: Runtime overhead for activating another process on another core or setting a event for a process on another core (inside a task or ISR) 
