...
| Structures | Containment |HwStructure |* |Hierarchic structure of the hardware model |
...
| Access Elements | Containment | AccessElement | * |Access element for a specific memory or processing unit |
...
A _Memory_ is a component of type _HwModule_ to express any kind memory like SRAM, DRAM, Flash, etc. in the model, caches are modeled separately. The _Memory_ element can be referenced as destination by an _HwAccessElement_.
...
|_. Attribute |_. Type |_. Value |_. Mul. |_. Description |
| Name | String | String | 1 | Name of the hardware feature |
| Value | Containment | Value | 1 | assigned factor to the coressponding feature |
...
h4. ProcessingUnitDefinition

The example in figure xxx is representative for any kind of definition in the model. This means for specifying a compute resource a _ProcessingUnitDefinition_ is created once which is then referenced by the number of _ProcessingUnit_ instances of this kind. For a _ProcessingUnitDefinition_ the references to _HwFeatures_ is on of the key concepts of the hardware model. The concept is explained in figure xxx.
...
The example in figure xxx is representative for any kind of definition in the model. This means for specifying a cache, a _CacheDefinition_ is created once which is then referenced by the number of _Cache_ instances of this kind.
...
h4. ConnectionHandlerDefinition

The example in figure xxx is representative for any kind of definition in the model. This means for specifying a bus or Interconnect etc., a _ConnectionHandlerDefinition_ is created once which is then referenced by the number of _ConnectionHandler_ instances of this kind.
