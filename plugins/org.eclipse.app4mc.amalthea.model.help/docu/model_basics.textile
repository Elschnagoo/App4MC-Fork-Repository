...
A time histogram allows for a deviation directly extracted from a trace. It represents a discrete distribution containing a limited number of entries (e.g. extracted from a trace). Each entry thereby is a __TimeInterval__ with the extra attribute __occurrences__ which holds the number instances whose time is within the interval. The intervals do not have to cover a continuous range nor do they need to have the same interval size. Time histograms are useful if there is a limited number of possibilities of valuations, which covers most practical systems. See the following figure for an example of two runnables having a time histogram deviation of their execution times.
