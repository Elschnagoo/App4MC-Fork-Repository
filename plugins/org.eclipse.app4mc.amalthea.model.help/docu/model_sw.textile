...
| __activation__ | Although runnables in the simulation can only be called by tasks, it is possible to specify an activation pattern for a runnable. |
...
At the __RunnableModeSwitch__ it is also possible to define a default path, which is executed if no condition of the __ModeSwitchEntries__ fits to the current mode settings.
...
A __Mode Switch__ uses the value of mode labels to decide which entry has to be executed. Therefore a mode condition is defined for each entry (via ModeConditionDisjunction). The first fulfilled condition determines the path to execute. 
