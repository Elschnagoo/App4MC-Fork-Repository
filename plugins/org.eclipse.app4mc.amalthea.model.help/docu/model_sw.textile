...
| __dataStability__ | Enumeration value to represent the data stability needs of the label. If set, it overwrites the global settings stated by the __OsDataDependency__, otherwise it inherits them (see <notextile><a href="./model_os.html#OsDataConsistency">OS Data Consistency</a></notextile>). |

h2. Channels

The channel element represents a data element. TODO
...
h3. Calls and AUTOSAR communication
...
| __dataStability__ | Describes the data stability needs. If set, it overwrites the label settings, otherwise it inherits them (see <notextile><a href="./model_os.html#OsDataConsistency">OS Data Consistency</a></notextile>). |
| __statistic__ | TODO |
| __transmissionPolicy__ | The following attributes reflect computing demand (time) depending on data: <ul><li> *ChunkSize*: Size of a part of an element, maximum is the element size. </li><li> *ChunkProcessingInstructions*: Number of instructions that will be executed to receive one chunk. </li></ul> The next attribute specifies the amount of data actually accessed by a runnable (required to analyze memory bandwidth demands): <ul><li> *ReceiveRatio*: Specify the ratio of each element that is actually received by the runnable in percent. Value must be between [0, 1], default value is 1.0 . </li></ul> |
...
h3. Runnable Probability Switch
...
Section (Vitual Memory Section) is used to group the memory elements (Labels, Runnables). This is acheived by associating the *Section* element to *Label* & *Runnable* elements
