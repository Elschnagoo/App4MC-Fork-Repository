...
The namespace for the model is "http://app4mc.eclipse.org/amalthea/0.7.0/sw".
...
table{text-align:left; padding:10px; border:1px solid black; background:#eee}.
...
Runnables do not have any parameters by calling them, as services could have. They can be initialized having labels or functions as parameter, using a reference or pointer to the target element (Remark: This is currently not represented in the model, but it is planned for one of the next major release).
