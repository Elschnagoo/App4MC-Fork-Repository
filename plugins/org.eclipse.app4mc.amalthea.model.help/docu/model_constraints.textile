...
The following picture shows a __RunnableSequencingConstraint__. The constrain has two runnable groups, visualized by the circles. In this case there is just one runnable in each group.
...
The Direction hereby is used to specify if the labels have to be read or written coherently. Moreover, the scope attribute defines the context of the coherent read or write requirement. Possible scopes are components, processses, and runnables.
...
* Process
* Runnable
This means, that it has to be garanteed that the values of labels are identical either within the runnable, the process, or the component in which the denoted labels are accessed.
...
h3. Event Synchronization Constraint
...
The parameter __multipleOccurrencesAllowed__ defines, whether for the constraint all occurences have to be considered or just the subsequent ones.
...
h3. Event Chain Synchronization Constraint
...
The __SynchronizationType__ defines which parts of the event chains have to be in sync, stimuls or response, and the width of a time window sets the allowed tolerance.
The parameter __multipleOccurrencesAllowed__ defines, whether for the constraint all occurences have to be considered or just the subsequent ones.

!../pictures/model_constraints_timing_ecsync.png!
...
A __DataConstraint__ is used to to define the mapping of label objects to memory units.

!../pictures/model_constraints_affinity_data.png!

h3. Process Affinity Constraints

A __ProcessConstraint__ is used to to define the mapping of process (Task or ISR) objects to processing cores or scheduling units.

!../pictures/model_constraints_affinity_process.png!

h3. Runnable Affinity Constraints

A __RunnableConstraint__ is used to to define the mapping of runnable objects to processes, call sequences, processing cores, or schedulin units.
