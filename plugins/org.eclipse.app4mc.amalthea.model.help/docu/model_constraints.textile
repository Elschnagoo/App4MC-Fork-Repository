...
Time Metrics are metrics that describe time intervals of an individual process instance or between two succeeding process instances and are defined as follows:
* __ActivateToActivate__: This metric indicates the distance between two successive activations of a task or isr. The __ActivateToActivate__ metric of process instance n measures the time between the activation moment of process instance n and that of instance n+1. 
* __CoreExecutionTime__
* __EndToEnd__: This metric indicates the time interval between two successive ends of a process. The __EndToEnd__ metric of process instance n measures the time between the termination moment of process instance n and that of instance n+1.
* __EndToStart__: This metric indicates the time interval between two successive process instances. The __EndToStart__ metric of process instance n measures the time between the termination moment of process instance n and the start moment of instance n+1.
...
* __Lateness__: This metric indicates whether a process instance misses its deadline. It measures the time between the termination moment of the process instance and its deadline. Thus, the resulting lateness is negative and indicates that no deadline miss occurred if the termination moment of the process instance is before the deadline and vice versa.
* __MemoryAccessTime__
* __NetExecutionTime__: The net execution time indicates the actual execution time of a process instance, i.e., the time it is occupying a processing unit. Thus, it measures the time from the start moment of a process instance to its termination moment excluding the time the process instance is interfered.
* __OsOverhead__
* __ParkingTime__
* __PollingTime__
* __ReadyTime__
...
* __RunningTime__
* __StartDelay__: This metric measures the delay of the start time of a process instance which is defined as the time interval between the activation moment of this process instance and its start moment.
* __StartToStart__: This metric indicates the time interval between two successive starts of a process. The __StartToStart__ metric of process instance n measures the time between the start moment of process instance n and that of instance n+1.
* __WaitingTime__: This metric measures the amount of time that a process instance spends waiting for the access of a resource.
