...
|priority|integer|TaskAllocation|Mapping Model|priority attribute is removed|
...
|parameter|AlgorithmParameter|Scheduler| OS model| parameter tag name is changed to parameterExtensions (_Also the new variable Type here is : ParameterExtensions_)|
...
|quartzes|List of Quartz| <ul><li>Core</li><li>ECU</li><li>HwComponent</li><li>Memory</li><li>Microcontroller</li><li>Network</li></ul>|Hardware Model|All Quartz elements from these model elements are moved to HwSystem element. <br><b>Note:</b>If there are nested Quartz elements, they are flattened and stored inside a single list|