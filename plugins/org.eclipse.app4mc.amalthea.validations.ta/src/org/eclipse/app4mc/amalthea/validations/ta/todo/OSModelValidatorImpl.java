...
public class OSModelValidatorImpl {
		...
		while (amaIter.hasNext()) {
			...
			if (elem instanceof Semaphore) {
				...
				if(false == (0 <= initialValue)) {
					issue(semaphore, AmaltheaPackage.eINSTANCE.getSemaphore_InitialValue());
				}
				int maxValue = semaphore.getMaxValue();
				if(false == (0 < maxValue)) {
					issue(semaphore, AmaltheaPackage.eINSTANCE.getSemaphore_MaxValue());
				}
	...
	public void checkSemaphoreReferences(final Amalthea amalthea) {
		...
		for (SemaphoreEvent event : events) {
			...
			if(false == found) {
				issue(event, AmaltheaPackage.eINSTANCE.getSemaphoreEvent_Entity());
			}
		}
		...
		for (SemaphoreAccess access : accesses) {
			...
			if(false == found) {
				issue(access, AmaltheaPackage.eINSTANCE.getSemaphoreAccess_Semaphore());
			}
		}
		...
		for (Component component : components) {
			if(null != osModel) {
				for(Semaphore semaphore1 : component.getSemaphores()) {
					...
					if(false == found) {
						issue(component, AmaltheaPackage.eINSTANCE.getComponent_Semaphores());
					}
				}
			}
		}
