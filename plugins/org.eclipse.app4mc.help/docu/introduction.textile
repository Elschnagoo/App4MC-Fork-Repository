...
The tool platform shall be capable to support all aspects of the development cycle. This addresses predominantly the automotive domain but is also applicable to telecommunication by extensions which deal with such systems in their native environment and integrated in a car. It shall be possible to simulate parallelized systems and give support for the debugging of multi-core systems. The tool chain will include data description tools and editors.
...
The result of the project is e an open tool platform in different aspects. On the one hand it is published under the Eclipse public license and on the other hand it will be open to be integrated with existing or new tools either on a company individual basis or with commercially available tools. In order to achieve this openness we will use clearly defined interfaces and an architecture which is specially built under these aspects.
