...
h2(#workflow-basic-components). Available Basic Components
...
|Using classes (API) from 3rd party libraries (added in the classpath of the plugin)inside custom workflow component java classes|{text-align:center;color:green}. *yes* |{text-align:center;color:red}. no(*) | <ul><li> *MWE2*: As new Java runtime is created, all the required plugins & jars are added in the classpath </li><li> *EASE*:  In the current implementation only the runtime from workbench is considered, jars from the classpath of a plugin (created newly in the runtime workbench) are not loaded in the runtime of EASE. Due to this reason, execution of the script fails with an exception java.lang.NoClassDefFoundError: <your used class from 3rd party lib> </li></ul> |
...
A sample workflow is available in the AMALTHEA example projects inside of @org.eclipse.app4mc.amalthea.example.democar.mapping/workflow/sample_workflow.js@. In general this workflow contains all steps as shown in the AMALTHEA examples tutorial "Task creation":./examples.html#example-task-creation and "Mapping example":./examples.html#example-mapping-1, but it can be executed in one script. Execute it by doing a right click and then __Run As -> EASE Script__.
