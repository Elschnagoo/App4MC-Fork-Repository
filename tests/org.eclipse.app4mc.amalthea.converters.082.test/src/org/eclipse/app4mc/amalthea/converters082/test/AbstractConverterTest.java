	...
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_082 = this.helper.isNS_AvailableIn_082(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.8.2 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_082);
			}
			else {
