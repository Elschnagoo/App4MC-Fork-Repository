...
public abstract class AbstractConverterTest {
	...
	protected List<ICache> buildCaches() {
		...
		final ConstraintElementsCacheBuilder cacheBuilder = new ConstraintElementsCacheBuilder();
	...
	/**
	 * This method is used to verify if the AMALTHEA namespaces used in this document are of AMALTHEA 0.8.1
	 *
	 * @param document
	 */
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_081 = this.helper.isNS_AvailableIn_081(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.8.1 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_081);
			}
			else {
