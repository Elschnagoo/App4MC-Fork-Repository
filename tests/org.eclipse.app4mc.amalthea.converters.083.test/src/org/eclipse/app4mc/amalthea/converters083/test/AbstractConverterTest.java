...
public abstract class AbstractConverterTest {
	...
	/**
	 * This method is used to verify if the AMALTHEA namespaces used in this document are of AMALTHEA 0.8.3
	 *
	 * @param document
	 */
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_083 = this.helper.isNS_AvailableIn_083(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.8.3 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_083);
			}
			else {
	...
	private Namespace[] getNameSpaces() {
		...
		final NameSpace_081[] ns4 = NameSpace_081.values();

		for (final NameSpace_081 namespace : ns4) {

			nsList.add(this.helper.getNS_081(namespace.name()));
		}

		final NameSpace_082[] ns5 = NameSpace_082.values();

		for (final NameSpace_082 namespace : ns5) {

			nsList.add(this.helper.getNS_082(namespace.name()));
		}
		
		final NameSpace_083[] ns6 = NameSpace_083.values();
