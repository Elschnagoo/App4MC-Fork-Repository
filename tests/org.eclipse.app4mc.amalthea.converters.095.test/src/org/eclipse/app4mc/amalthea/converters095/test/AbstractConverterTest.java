...
package org.eclipse.app4mc.amalthea.converters095.test;
...
public abstract class AbstractConverterTest {
	...
	protected HelperUtils_094_095 helper;
	...
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_095 = this.helper.isNS_AvailableIn_095(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.9.5 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_095);
			}
			else {
