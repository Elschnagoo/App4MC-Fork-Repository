	...
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_071 = this.helper.isNS_AvailableIn_071(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.7.1 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_071);
