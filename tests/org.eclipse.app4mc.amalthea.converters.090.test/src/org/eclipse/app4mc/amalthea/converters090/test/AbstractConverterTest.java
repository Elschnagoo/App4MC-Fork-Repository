...
public abstract class AbstractConverterTest {
	...
	/**
	 * This method is used to verify if the AMALTHEA namespaces used in this document are of AMALTHEA 0.9.0
	 *
	 * @param document
	 */
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_090 = this.helper.isNS_AvailableIn_090(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.9.0 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_090);
			}
			else {
