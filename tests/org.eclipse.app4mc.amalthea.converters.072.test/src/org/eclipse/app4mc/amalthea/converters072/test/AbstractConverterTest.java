	...
	protected void namespaceVerification(final Document document) {
		...
		for (final Namespace namespace : namespacesInScope) {
			...
			if (isAmaltheaNameSpacePrefix) {
				final boolean ns_AvailableIn_072 = this.helper.isNS_AvailableIn_072(namespace);

				assertTrue("Expected namespace from AMALTHEA 0.7.2 : " + namespace.getURI() + "  --> "
						+ document.getBaseURI(), ns_AvailableIn_072);
